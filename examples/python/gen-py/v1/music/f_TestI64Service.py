...
class Client(Iface):
    ...
    def _test(self, ctx, thing):
        self._send_test(ctx, thing)
        return self._recv_test(ctx)

    def _send_test(self, ctx, thing):
        oprot = self._oprot
    ...
    def _recv_test(self, ctx):
        ...
        x = TApplicationException(TApplicationExceptionType.MISSING_RESULT, "test failed: unknown result")
        raise x
...
class _test(FProcessorFunction):
    ...
    def process(self, ctx, iprot, oprot):
        ...
        result = test_result()
        try:
            result.success = self._handler([ctx, args.thing])
            ...
            with self._lock:
                _write_application_exception(ctx, oprot, "test", ex_code=TApplicationExceptionType.INTERNAL_ERROR, message=e.message)
            raise
        with self._lock:
            try:
                ...
                if e.type == TTransportExceptionType.REQUEST_TOO_LARGE:
                    raise _write_application_exception(ctx, oprot, "test", ex_code=TApplicationExceptionType.RESPONSE_TOO_LARGE, message=e.args[0])
                else:
                    raise e
